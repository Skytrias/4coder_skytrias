...
function void
_cpp_parse_type_structure(Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent){
	generic_parse_inc(state);
...
function void
_cpp_parse_function(Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent){
	Token *token = token_it_read(&state->it);
	generic_parse_inc(state);
	generic_parse_skip_soft_tokens(index, state);
	if (state->finished){
...
function void
odin_parse_generic(Application_Links *app, Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent, i64 search, i64 note) {
	Token *token = token_it_read(&state->it);
	generic_parse_inc(state);
	generic_parse_skip_soft_tokens(index, state);
	if (state->finished){
    ...
    i64 line_start = get_line_number_from_pos(app, index->buffer, peek->pos);
    
    if (peek != 0 && (peek->sub_kind == TokenCppKind_ColonColon || peek->kind == TokenCppKind_ColonColon)) {
		for (; peek != 0;) {
			generic_parse_inc(state);
            peek = token_it_read(&state->it);
			i64 current_line = get_line_number_from_pos(app, index->buffer, peek->pos);
            
            if (peek == 0 || state->finished || line_start != current_line) {
				break;
			}
			
			if (peek->sub_kind == search) {
				index_new_note(index, state, Ii64(token), note, parent);
				break;
			}
		}
	}
...
function void
odin_parse_generic(Application_Links *app, Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent) {
	Token *token = token_it_read(&state->it);
	Token *reset_point = token; // NECESSARY
	generic_parse_inc(state);
	generic_parse_skip_soft_tokens(index, state);
	if (state->finished){
		return;
    ...
    i64 line_start = get_line_number_from_pos(app, index->buffer, token->pos);
    ...
    if (peek != 0 && (peek->sub_kind == TokenCppKind_ColonColon || peek->kind == TokenCppKind_ColonColon)) {
		for (; peek != 0;) {
			...
			if (peek->sub_kind == TokenCppKind_proc) {
				index_new_note(index, state, Ii64(token), CodeIndexNote_Function, parent);
				break;
			}
        }
...
function void
odin_parse_consts(Application_Links *app, Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent) {
	Token *token = token_it_read(&state->it);
	generic_parse_inc(state);
	generic_parse_skip_soft_tokens(index, state);
	if (state->finished){
    ...
    i64 start_line = get_line_number_from_pos(app, index->buffer, token->pos);
    i64 colons = 0;
    ...
    for (;;) {
        ...
        if (peek->kind == TokenCppKind_Colon || peek->sub_kind == TokenCppKind_Colon) {
            colons += 1;
        }
        
        if (peek->kind == TokenCppKind_ColonColon || peek->sub_kind == TokenCppKind_ColonColon) {
            colons += 2;
        }
        
        if (colons > 1 && (peek->sub_kind == TokenCppKind_Semicolon || peek->kind == TokenCppKind_Semicolon)) {
            //Scratch_Block scratch(app);
            //String_Const_u8 text = push_stringf(scratch, "%d \n", (colons));
            //print_message(app, text);
            index_new_note(index, state, Ii64(token), CodeIndexNote_Constant, parent);
            break;
        }
...
function Code_Index_Nest*
generic_parse_scope(Application_Links *app, Code_Index_File *index, Generic_Parse_State *state);
...
generic_parse_scope(Application_Links *app, Code_Index_File *index, Generic_Parse_State *state){
	...
	for (;;){
        ...
        if (token->kind == TokenBaseKind_Identifier) {
            //odin_parse_generic(app, index, state, 0);
        }
...
function b32
generic_parse_full_input_breaks(Application_Links *app, Code_Index_File *index, Generic_Parse_State *state, i32 limit){
	b32 result = false;
	...
	for (;;){
        ...
        // TODO(Skytrias): do function check in scope
            ...
            if (token->sub_kind == TokenCppKind_Identifier) {
                odin_parse_generic(app, index, state, 0);
                odin_parse_consts(app, index, state, 0);
            } else {
                generic_parse_inc(state);
            }
		}
