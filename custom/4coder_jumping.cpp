...
parse_jump_location(String_Const_u8 line){
	...
	if (!jump.is_ms_style && first_colon != 0 && second_colon != 0 && third_colon != 0 && first_colon < line.size && third_colon < line.size) {
		...
		String_Const_u8 location_str = string_prefix(line, first_colon);
		jump.location.file = location_str;
		
		String_Const_u8 line_number = string_skip(string_prefix(line, second_colon), first_colon + 1);
	...
	if (!jump.is_ms_style) {
		...
		u64 colon_seperator = left_paren_pos + string_find_first(string_skip(line, left_paren_pos), ':');
		...
		String_Const_u8 line_number = string_skip(string_prefix(line, colon_seperator), left_paren_pos + 1);
		jump.location.line = (i32) string_to_integer(line_number, 10); 
