...
// solarized dark
ARGB_Color argb_function = 0xFF268bd2;
ARGB_Color argb_type = 0xFF859900; // struct, enum, union
ARGB_Color argb_const = 0xffcb4b16; // "::" ending with ";"
ARGB_Color argb_highlight = 0x44cb4b16;
ARGB_Color argb_highlight_minor = 0x33cb4b16;
...
code_index_update_tick(Application_Links *app){
	...
	for (Buffer_Modified_Node *node = global_buffer_modified_set.first;
		...
		generic_parse_full_input_breaks(app, index, &state, max_i32);
...
default_render_buffer(Application_Links *app, View_ID view_id, Face_ID face_id,
	...
	if (token_array.tokens != 0){
		...
		Scratch_Block scratch(app);
		//ARGB_Color argb_struct = 0xFFFF00FF;
		...
		for (;;){
			...
			if (note != 0) {
				if (note->note_kind == CodeIndexNote_Function) {
					paint_text_color(app, text_layout_id, Ii64_size(token->pos, token->size), argb_function);
				} else if (note->note_kind == CodeIndexNote_Type) {
					paint_text_color(app, text_layout_id, Ii64_size(token->pos, token->size), argb_type);
				} else if (note->note_kind == CodeIndexNote_Constant) {
					paint_text_color(app, text_layout_id, Ii64_size(token->pos, token->size), argb_const);
				}
			}
		}
#endif
				...
				if (token->kind == TokenBaseKind_Identifier)
				{
					...
					if (range_contains(token_range, view_get_cursor_pos(app, view_id)))
					{
						F4_RenderRangeHighlight(app, view_id, text_layout_id,
																		token_range, F4_RangeHighlightKind_Whole,
																		argb_highlight);
					}
...
parse_async__inner(Async_Context *actx, Buffer_ID buffer_id,
	...
	for (;;){
		if (generic_parse_full_input_breaks(app, index, &state, limit_factor)){
			break;
		}
