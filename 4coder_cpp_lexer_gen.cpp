/*
4coder_lex_gen_cpp.cpp - Model definition for a rust lexer.
*/
...
build_language_model(void){
	...
	sm_char_name('&', "And");
	sm_char_name('|', "Or");
	// NOTE(Skytrias): renamed because of rust
	sm_char_name('%', "Modulo");
	sm_char_name('^', "Xor");
	sm_char_name('?', "Ternary");
	sm_char_name('/', "Div");
	
	// rust Direct Toke Kinds
	...
	// stability with old cpp files
	sm_key("Typedef");
	
	// rust taken from their website
	// default
	sm_key("Bool");
	sm_key("F32");
	sm_key("F64");
