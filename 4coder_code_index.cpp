...
// NOTE(Skytrias): CHANGES REQUIRE CHANGES TO code_index.h, add i64 line_number in Code_Index_Nest
...
// NOTE(Skytrias): All parses were changed to include line_numbers of their found location
function Code_Index_Nest*
generic_parse_statement(Application_Links *app, Code_Index_File *index, Generic_Parse_State *state);
...
generic_parse_scope(Application_Links *app, Code_Index_File *index, Generic_Parse_State *state){
    ...
    for (;;){
        ...
        {
            Code_Index_Nest *nest = generic_parse_statement(app, index, state);
            nest->parent = result;
            nest->line_number = get_line_number_from_pos(app, index->buffer, token->pos);
			code_index_push_nest(&result->nest_list, nest);
        }
...
layout_index__inner(Application_Links *app, Arena *arena, Buffer_ID buffer, Range_i64 range, Face_ID face, f32 width, Code_Index_File *file, Layout_Wrap_Kind kind){
    ...
    if (text.size == 0){
        ...
        {
			// NOTE(Skytrias): indent all dots to the right if theyre alone
			if (*ptr == '.') {
				lr_tb_advance_x_without_item(&pos_vars, 4.0f * metrics.space_advance);
			}
        ...
        for (; ptr < end_ptr; ptr += 1){
            if (!character_is_whitespace(*ptr)){
                ...
                token_score += layout_token_score_wrap_token(&new_wrap_token_pair, TokenCppKind_ModuloEq);
